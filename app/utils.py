...
def add_city_to_df(df):
	df["lon"], df["lat"] = df.parallel_apply(lambert93_to_lat_long, axis=1, result_type='expand').T.values
...
def get_cities(file):
	res = requests.post('https://api-adresse.data.gouv.fr/reverse/csv/', files={'data': file})
		...
		if user_mcc and user_mnc:
			...
			for country in json_data.keys():
				if user_mcc == json_data[country]["MCC"] and user_mnc == json_data[country]["MNC"]:
					...
					match =  json_data[country]["NETWORK"]
